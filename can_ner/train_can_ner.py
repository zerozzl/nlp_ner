...
def train(args, dataset, dataloader, model, optimizer, lr_scheduler):
    ...
    for batch, data in enumerate(dataloader):
        ...
        loss = model(sents, masks, sents_len, seg_tags, decode=False, tags=ner_tags)
        loss = loss.mean()
...
def evaluate(args, dataloader, id_to_tag, model, label_type):
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            ...
            preds = model(sents, masks, sents_len, seg_tags)
...
def main(args):
    ...
    if args.pretrained_model_path is not None:
        ...
        model = NER(len(tag_to_id), len(char_to_id), args.char_embed_size, args.input_dropout_rate,
                    args.hidden_dim, args.window_size, args.hidden_dropout_rate,
                    args.use_seg_embed, len(SEG_TAG_TO_ID), args.seg_embed_size,
                    args.use_bigram_embed, len(bigram_to_id), args.bigram_embed_size,
                    args.use_pos_embed, args.use_rnn, args.use_crf, args.embed_freeze, args.use_cpu)
        model = model.cpu() if args.use_cpu else model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', dest='output_path',
                        default='../runtime/can_ner')
    parser.add_argument('--use_rnn', dest='use_rnn', type=bool,
                        default=False)
    parser.add_argument('--use_crf', dest='use_crf', type=bool,
                        default=False)
    parser.add_argument('--use_pos_embed', dest='use_pos_embed', type=bool,
                        default=False)
