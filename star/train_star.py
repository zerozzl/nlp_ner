...
def data_collate_fn(data):
    ...
    heads = torch.LongTensor(np.array(data[:, 5].tolist()))
    tails = torch.LongTensor(np.array(data[:, 6].tolist()))

    return sents, seg_tags, ner_tags, masks, heads, tails, sents_len
...
def train(args, dataset, dataloader, model, optimizer, lr_scheduler):
    ...
    for batch, data in enumerate(dataloader):
        optimizer.zero_grad()
        sents, seg_tags, ner_tags, masks, heads, tails, _ = data
        ...
        heads = heads.cpu() if args.use_cpu else heads.cuda()
        tails = tails.cpu() if args.use_cpu else tails.cuda()

        loss = model(sents, masks, heads, tails, seg_tags, decode=False, tags=ner_tags)
        loss = loss.mean()
...
def evaluate(args, dataloader, id_to_tag, model, label_type):
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            sents, seg_tags, ner_tags, masks, heads, tails, sents_len = data
            ...
            preds = model(sents, masks, heads, tails, seg_tags)
...
def main(args):
    ...
    if args.pretrained_model_path is not None:
        ...
        model = NER(len(tag_to_id), len(char_to_id), args.char_embed_size, args.input_dropout_rate,
                    args.cycle_num, args.num_attention_heads, args.attn_dropout_rate, args.fc_dropout_rate,
                    args.use_seg_embed, len(SEG_TAG_TO_ID), args.seg_embed_size,
                    args.use_bigram_embed, len(bigram_to_id), args.bigram_embed_size,
                    args.use_crf, args.add_boundary, args.embed_freeze)
        model = model.cpu() if args.use_cpu else model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_bigram_embed', dest='use_bigram_embed', type=bool,
                        default=True)
    parser.add_argument('--add_boundary', dest='add_boundary', type=bool,
                        default=True)
    ...
    parser.add_argument('--input_dropout_rate', dest='input_dropout_rate', type=float,
                        default=0.2)
    parser.add_argument('--fc_dropout_rate', dest='fc_dropout_rate', type=float,
                        default=0.2)
    parser.add_argument('--cycle_num', dest='cycle_num', type=int,
                        default=6)
    parser.add_argument('--num_attention_heads', dest='num_attention_heads', type=int,
                        default=5)
    parser.add_argument('--attn_dropout_rate', dest='attn_dropout_rate', type=float,
                        default=0.1)
