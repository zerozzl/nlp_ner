...
def main(args):
    ...
    if args.pretrained_model_path is not None:
        ...
        model = NER(len(tag_to_id), len(char_to_id), args.char_embed_size, args.input_dropout_rate,
                    args.num_layers, args.n_head, args.head_dims,
                    args.fc_dropout_rate, args.attn_dropout_rate, args.after_norm, args.attn_scale,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fc_dropout_rate', dest='fc_dropout_rate', type=float,
                        default=0.4)
    parser.add_argument('--num_layers', dest='num_layers', type=int,
                        default=2)
    parser.add_argument('--n_head', dest='n_head', type=int,
                        default=6)
    parser.add_argument('--head_dims', dest='head_dims', type=int,
                        default=80)
    parser.add_argument('--attn_dropout_rate', dest='attn_dropout_rate', type=float,
                        default=0.15)
    parser.add_argument('--after_norm', dest='after_norm', type=bool,
                        default=True)
    parser.add_argument('--attn_scale', dest='attn_scale', type=int,
                        default=False)
